...
const AdminOutingClassPage = () => {
  ...
  const fetchOutingClass = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.error("Failed to fetch outing class:", result.message);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingItem
        ? `/api/outing-class/${editingItem.id}`
        : "/api/outing-class";
      ...
      if (result.success) {
        alert(
          editingItem
            ? "Outing class berhasil diupdate!"
            : "Outing class berhasil ditambahkan!"
        );
        setShowModal(false);
        setEditingItem(null);
        setFormData({
  ...
  const handleDelete = async (id) => {
    if (!confirm("Apakah Anda yakin ingin menghapus outing class ini?")) {
      return;
    }
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 text-[1.4rem]">
            Kelola data outing class sekolah
          </p>
        </div>
        <button
          onClick={() => {
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {!loading && outingClass.length === 0 ? (
                <tr>
                  <td
                    colSpan="7"
                    className="px-6 py-8 text-[1.4rem] text-center text-gray-500"
                  >
                    Belum ada data outing class
                  </td>
                </tr>
              ) : (
                outingClass.map((item, index) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-[1.4rem] text-gray-600 flex items-center">
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-[2rem] font-bold text-gray-800">
                {editingItem ? "Edit Outing Class" : "Tambah Outing Class"}
              </h2>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-[1.4rem] font-[500] text-gray-700 mb-2">
                  Judul Outing Class <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  name="judul"
                  ...
                  placeholder="Masukkan judul outing class"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows="4"
                  className="w-full text-[1.4rem] px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 placeholder:text-[1.4rem]"
                  placeholder="Masukkan deskripsi outing class"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full text-[1.4rem] px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 placeholder:text-[1.4rem]"
                  placeholder="Masukkan lokasi outing class"
                />
              </div>
              ...
              <div>
                ...
                <CldUploadWidget
                  ...
                  options={{
                    folder: "system-sekolah/outing-class",
                    resourceType: "image",
                    maxFiles: 1,
                    maxFileSize: 2000000,
                    sources: ["local"],
...
export default AdminOutingClassPage;
